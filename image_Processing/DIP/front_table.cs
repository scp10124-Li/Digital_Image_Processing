...
namespace DIP
{
    ...
    public partial class front_table : Form
    {
        ...
        [DllImport("direction.dll", CallingConvention = CallingConvention.Cdecl)]
        unsafe static extern void rota(int* f0, int w, int h, int* g0, double theta_deg);
        ...
        private static Bitmap array2bmp(int[] ImgData, int Width, int Height)
        {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < Height; y++)
                {
                    ...
                    {
                        int val = ImgData[x + Width * y];
                        val = Clamp(val, 0, 255);  // 防止 overflow
            ...
            foreach (MSForm cF in MdiChildren)
            {
                if (cF.Focused)
                {
                    ...
                    unsafe
                    {
                        fixed (int* f0 = f)
                        {
                            using (rotation form2 = new rotation(f0, w, h, null, &theta))
                            {
                                ...
                                {
                                    // 從 rotation 表單取得輸出影像
                                    int[] rotatedG = form2.OutputArray;
                                    int newW = form2.OutputWidth;
                                    int newH = form2.OutputHeight;

                                    NpBitmap = array2bmp(rotatedG, newW, newH);
                                }
                                else
                                {
                                    // 若取消則顯示原圖
                                    NpBitmap = array2bmp(f, w, h);
                                }
                            }
                        }
                    }
